...
def test_exec(config, ssh_con):
    ...
    for i in range(config.bucket_count):
        for oc, size in list(config.mapped_sizes.items()):
            ...
            s3_object_name = utils.gen_s3_object_name(buckets[i].name, oc)
            s3_object_path = os.path.join(TEST_DATA_PATH, s3_object_name)
            reusable.upload_object(
                s3_object_name, buckets[i], TEST_DATA_PATH, config, user_info
            )
        ...
        op = utils.exec_shell_cmd("radosgw-admin bucket stats --bucket %s" % buckets[i].name)
